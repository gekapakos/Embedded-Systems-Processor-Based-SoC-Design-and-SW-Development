...
// Instantiation of Axi Bus Interface S00_AXI
    ...
    always @ (posedge s00_axi_aclk) begin
        if (!s00_axi_aresetn) begin
            ...
            if (s00_axi_wdata === 32'd10) begin// && s00_axi_wvalid == 1) begin
                button <= 1;
                count <= count + 24'd1;
            end
            else if (count == 24'd10_000_001) begin
                button <= 0;
                count <= 24'd0;
            end
            else begin 
    ...
    fpadd_system fpadd_system_inst(.clk(s00_axi_aclk), .rst(reset), .noisy_level(level), .leds(leds), 
